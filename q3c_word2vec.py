...
def naive_softmax_loss_and_gradient(
    ...
    grad_center_vec = outside_vectors.T.dot(y_hat) # multiply outside vectors with y hat
    grad_center_vec -= outside_vectors[outside_word_idx] # remove u_o

    y_hat_minus_1 = y_hat

    y_hat_minus_1[outside_word_idx] -= 1 # calc yhat -1 for the case of w is o
    grad_outside_vecs = y_hat_minus_1[:, np.newaxis].dot(np.array([center_word_vec])) # multiply for all outside vectors
...
def neg_sampling_loss_and_gradient(
    ...
    u_o_mul_v_c = outside_vectors[outside_word_idx].dot(center_word_vec) # u_o * v_c
    u_k_mul_v_c = outside_vectors[neg_sample_word_indices].dot(center_word_vec) # u_k * c_v

    sig_u_o = sigmoid(u_o_mul_v_c)
    sig_u_k = sigmoid(-u_k_mul_v_c)

    loss = - np.log(sig_u_o) - np.sum(np.log(sig_u_k))
    grad_center_vec = (sig_u_o - 1) * outside_vectors[outside_word_idx] + \
                      np.sum((1 - sig_u_k)[:, np.newaxis] * outside_vectors[neg_sample_word_indices], axis=0)

    grad_outside_vecs = np.zeros(outside_vectors.shape)
    grad_outside_vecs[outside_word_idx] = (sig_u_o - 1) * center_word_vec # add term from theory to vector
    for i, neg_index in enumerate(neg_sample_word_indices):
        grad_outside_vecs[neg_index] += (1 - sig_u_k[i]) * center_word_vec # remember negative can appear multiple times
...
def skipgram(current_center_word, outside_words, word2ind,
    ...
    for word in outside_words:
        ...
        center_vector = center_word_vectors[word2ind[current_center_word]]

        loss_a, grad_center_vecs_a, grad_outside_vectors_a = word2vec_loss_and_gradient(center_vector, word2ind[word], outside_vectors, dataset)
        loss += loss_a
        grad_center_vecs[word2ind[current_center_word]] += grad_center_vecs_a
        grad_outside_vectors += grad_outside_vectors_a
